      ...
      forM_
        ...
        , (AchromaticCodel Cyan Dark, AchromaticCodel Green Normal, OutChar, Subtract)
          ...
          let expectedGraph = SyntaxGraph $ IM.fromList [ ( 0
                                                          , Block $ M.fromList [ let dpcc = DPCC DPRight CCLeft in (dpcc, NextBlock (command12) dpcc 1)
                                                                               , let dpcc = DPCC DPRight CCRight in (dpcc, NextBlock (command12) dpcc 1)
                                                                               ]
                                                          )
                                                        , ( 1
                                                          , Block $ M.fromList [ let dpcc = DPCC DPLeft CCLeft in (dpcc, NextBlock (command21) dpcc 0)
                                                                               , let dpcc = DPCC DPLeft CCRight in (dpcc, NextBlock (command21) dpcc 0)
                                                                               ]
                                                          )
                                                        ]
